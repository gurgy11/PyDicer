...
def register():
    if request.method == 'POST':
        ...
        # Create new address
        address_created, address_id = a_controller.create()

        if address_created is not True:
...
def login():
    request_errors = request.args.get('errors')
    if len(request_errors) > 0:
        return render_template('auth/login.html', title='DicePy - Login', errors=request_errors)
    ...
    if request.method == 'POST':
        ...
        # Login
        u_controller.login(user_form_data)
